...
class Phi35MiniInstruct(ChatbotBase):
    """
        Chatbot class for Phi-3.5-MoE-instruct model using Azure OpenAI.
    """
    ...
    def __init__(self, remote_tls_port=443):
        ...
        if not os.getenv('AZURE_PHI3_ENDPOINT'):
            raise ValueError('AZURE_PHI3_ENDPOINT is not set in the environment variables.')
        if not os.getenv('AZURE_PHI3_API_KEY'):
            raise ValueError('AZURE_PHI3_API_KEY is not set in the environment variables.')
        ...
        self._client = AzureOpenAI(
            ...
            api_key=os.getenv('AZURE_PHI3_API_KEY'),
