...
class AzureGPT41Nano(ChatbotBase):
    """
    ...
    def __init__(self, remote_tls_port=443):
        ...
        if not os.getenv('AZURE_OPENAI_ENDPOINT_41'):
            raise ValueError('AZURE_OPENAI_ENDPOINT_41 is not set in the environment variables.')
        if not os.getenv('AZURE_OPENAI_API_KEY_41'):
            raise ValueError('AZURE_OPENAI_API_KEY_41 is not set in the environment variables.')
