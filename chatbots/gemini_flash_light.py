...
class GeminiFlashLight(ChatbotBase):
    """
        Gemini 2.0 Flash Light chatbot.
    """
    ...
    def send_prompt(self, prompt, temperature):
        ...
        # Send prompt
        response = []
        stream = self._client.chat.completions.create(extra_body={}, model='gemini-2.0-flash-lite', messages=[ { 'role': 'user', 'content': prompt } ], stream=True, temperature=temperature)
        for chunk in stream:
